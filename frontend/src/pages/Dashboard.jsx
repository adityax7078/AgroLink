...
export default function Dashboard() {
  ...
  const [role, setRole] = useState(() => {
    ...
    if (currentUser?.role === 'farmer' || currentUser?.role === 'processor') {
  ...
  const handlePublishListing = (e) => {
    ...
    const qtyVal = Number(quantity);
    const priceVal = Number(expectedPrice);
  ...
  const handleSaveEdit = (e) => {
    ...
    const qtyVal = Number(editQuantity);
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950/20 min-h-screen py-10 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
                <div>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-550 uppercase tracking-wider">Active Listings</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-2">
                    {listings.filter(l => l.farmer === currentUser?.email).length} Crops
                  </p>
                </div>
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
                <div>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-555 uppercase tracking-wider">Earnings Sourced</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-2">
                    ₹{orders.filter(o => o.seller === currentUser?.email && o.status === 'Delivered').reduce((acc, curr) => acc + parseTotal(curr.total), 0).toLocaleString('en-IN')}
                  </p>
                </div>
                <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-xl">
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
                <div>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-555 uppercase tracking-wider">Pending Orders</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-2">
                    {orders.filter(o => o.seller === currentUser?.email && o.status === 'Pending').length} Orders
                  </p>
                </div>
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
                <div>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-555 tracking-wider uppercase">Completed Orders</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-2">
                    {orders.filter(o => o.seller === currentUser?.email && (o.status === 'Delivered' || o.status === 'Accepted')).length} Trades
                  </p>
                </div>
                <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-xl">
            ...
            <div className="lg:col-span-5 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-6 transition-colors">
              ...
              <form onSubmit={handlePublishListing} className="space-y-4 text-sm">
                <div>
                  <label className="block text-sm font-semibold text-slate-600 dark:text-slate-355 mb-1.5">Crop Type</label>
                  <select
                    value={cropType}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  />
                  <div>
                    <label className="block text-sm font-semibold text-slate-600 dark:text-slate-355 mb-1.5">Unit</label>
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-sm font-semibold text-slate-600 dark:text-slate-355">Expected Price (₹)</label>
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={triggerAISuggestion}
                      className="flex items-center gap-1.5 text-emerald-600 dark:text-emerald-455 hover:text-emerald-700 bg-emerald-50 dark:bg-slate-900 border border-emerald-100 dark:border-slate-800 hover:border-emerald-200 py-1 px-2 text-[11px] cursor-pointer"
                    >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {orders.filter(o => o.seller === currentUser?.email).map((ord, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                          ...
                          <td className="py-3 text-slate-605 dark:text-slate-400">{ord.processor}</td>
                            ...
                            <span
                              className={`inline-flex px-2 py-1 rounded-lg text-xs font-bold ${
                                ord.status === 'Accepted'
                                  ...
                                  : ord.status === 'Pending'
                                  ? 'bg-amber-50 dark:bg-amber-955/30 text-amber-700 dark:text-amber-400 border border-amber-100 dark:border-amber-900/50'
                            ...
                            <div className="flex items-center justify-center gap-1.5">
                              {ord.status === 'Pending' && (
                                <>
                                  <button
                                    ...
                                    className="px-2 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-[11px] font-bold shadow-sm cursor-pointer"
                                  >
                                    Accept
                                  </button>
                                  <button
                                    onClick={() => handleUpdateOrderStatus(ord.id, 'Rejected')}
                                    className="px-2 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-[11px] font-bold shadow-sm cursor-pointer"
                                  >
                                    Reject
                                  </button>
                                </>
                              ...
                              {ord.status === 'Accepted' && (
                                <button
                                  ...
                                  className="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-[11px] font-bold shadow-sm cursor-pointer"
                                >
                                  Mark Delivered
                                </button>
                              )}
                              {ord.status !== 'Pending' && ord.status !== 'Accepted' && (
                                <span className="text-slate-400 dark:text-slate-655 text-xs font-light">Completed</span>
                              )}
                            </div>
                          </td>
                        </tr>
                      ))}
                      {orders.filter(o => o.seller === currentUser?.email).length === 0 && (
                        <tr>
                          <td colSpan="6" className="py-8 text-center text-slate-400 font-light select-none bg-slate-50/50 dark:bg-slate-900/50 rounded-2xl">
                            No procurement orders received yet. Once processors place order contracts, they will show up here.
                          </td>
                        </tr>
                      )}
                    </tbody>
            ...
            <div className="lg:col-span-12 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm transition-colors mt-4">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800 text-slate-400 dark:text-slate-505 font-semibold">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {listings.filter(l => l.farmer === currentUser?.email).map((list, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="py-3 text-slate-550">{list.quantity} {list.unit}s</td>
                        ...
                        <td className="py-3 text-center">
                          <div className="flex items-center justify-center gap-2.5">
                            <button
                              ...
                              className="text-blue-600 hover:text-blue-800 dark:hover:text-blue-400 flex items-center gap-1 transition-colors cursor-pointer"
                            ...
                            <button
                              ...
                              className="text-red-600 hover:text-red-800 dark:hover:text-red-400 flex items-center gap-1 transition-colors cursor-pointer"
                    ...
                    {listings.filter(l => l.farmer === currentUser?.email).length === 0 && (
                      <tr>
                        <td colSpan="6" className="py-8 text-center text-slate-400 font-light select-none bg-slate-50/50 dark:bg-slate-900/50 rounded-2xl">
                          You have no active harvest listings. Register your crop using the form on the left to start receiving direct procurement offers!
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        {role === 'processor' && (
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-12 grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
                <div>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-550 uppercase tracking-wider">Active Procurement</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-2">
                    {orders.filter(o => o.processor === currentUser?.email && o.status === 'Accepted').length} Contracts
                  </p>
                </div>
                <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-xl">
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
                <div>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-555 uppercase tracking-wider">Total Investment</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-2">
                    ₹{orders.filter(o => o.processor === currentUser?.email).reduce((acc, curr) => acc + parseTotal(curr.total), 0).toLocaleString('en-IN')}
                  </p>
                </div>
                <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-xl">
                  <DollarSign className="h-5 w-5" />
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
                <div>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-555 uppercase tracking-wider">Verified Farms Connected</p>
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-2">
                    {new Set(orders.filter(o => o.processor === currentUser?.email).map(o => o.seller)).size} Farmers
                  </p>
                </div>
                <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-xl">
                  <CheckCircle className="h-5 w-5" />
            ...
            <div className="lg:col-span-12 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm transition-colors">
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-550 mt-1">Track orders and communicate with farmer partners.</p>
                </div>
                <span className="text-xs bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-450 border border-emerald-200/30 px-3 py-1.5 rounded-xl font-bold truncate max-w-[250px]" title={currentUser?.email}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {orders.filter(o => o.processor === currentUser?.email).map((ord, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="py-3 font-semibold text-slate-800 dark:text-slate-200">{ord.id}</td>
                        <td className="py-3 text-slate-700 dark:text-slate-350 font-medium">{ord.crop}</td>
                        <td className="py-3 text-slate-650 dark:text-slate-400">{ord.seller}</td>
                        <td className="py-3 text-slate-505">{ord.qty}</td>
                        <td className="py-3 text-slate-800 dark:text-slate-100 font-bold">{ord.total}</td>
                        <td className="py-3 text-slate-505">{ord.date}</td>
                        <td className="py-3 text-center">
                          <span
                            className={`inline-flex px-2 py-1 rounded-lg text-xs font-bold ${
                              ord.status === 'Accepted'
                                ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-450 border border-emerald-100 dark:border-emerald-900/50'
                                : ord.status === 'Pending'
                        ...
                        <td className="py-3 text-center">
                          <Button
                            ...
                            className="flex items-center gap-1.5 mx-auto bg-slate-50 dark:bg-slate-800 hover:bg-emerald-50 hover:text-emerald-755 py-1 px-2.5 text-xs cursor-pointer"
                          >
                    ...
                    {orders.filter(o => o.processor === currentUser?.email).length === 0 && (
                      <tr>
                        <td colSpan="8" className="py-8 text-center text-slate-400 font-light select-none bg-slate-50/50 dark:bg-slate-900/50 rounded-2xl">
                          No procurement history found. Procure harvests below.
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <div className="lg:col-span-12 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm transition-colors mt-4">
              ...
              <p className="text-xs text-slate-450 dark:text-slate-555 mb-6">Select a harvest published by a farmer to place an instant procurement order contract offer.</p>
                  ...
                  <div key={list.id} className="p-5 border border-slate-250/70 dark:border-slate-800 rounded-2xl bg-slate-50/50 dark:bg-slate-900/60 hover:border-emerald-200 dark:hover:border-emerald-900 flex flex-col justify-between transition-colors">
                    <div>
                      <div className="flex justify-between items-start mb-3">
                        <span className="px-2 py-0.5 rounded-lg bg-emerald-50 dark:bg-emerald-955 text-emerald-600 dark:text-emerald-405 font-semibold text-xs border border-emerald-100/50">
                          {list.badge}
                        </span>
                      ...
                      <p className="text-xs text-slate-455 mb-3">{list.description}</p>
        ...
        >
          <div className="space-y-4 text-sm text-slate-600 dark:text-slate-355">
            <p>Are you sure you want to permanently delete this crop listing? This action is destructive and cannot be undone.</p>
            <div className="flex gap-3 justify-end pt-3">
              ...
              <Button
                onClick={() => {
                  const id = deletingListingId;
                  setDeletingListingId(null);
                  executeDeleteListing(id);
                }}
                className="px-4 py-2 text-xs bg-red-600 hover:bg-red-750 text-white font-bold cursor-pointer"
            ...
            aiResult && (
              <div className="space-y-6 text-sm text-slate-600 dark:text-slate-350">
                <div className="text-center p-4 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40 rounded-2xl space-y-1">
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-550 font-medium">per quintal</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800/80">
                    <p className="text-xs text-slate-400 dark:text-slate-550 font-medium">Market Trend</p>
                    <p className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">{aiResult.marketTrend}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800/80">
                    <p className="text-xs text-slate-400 dark:text-slate-550 font-medium">Factory Demand</p>
